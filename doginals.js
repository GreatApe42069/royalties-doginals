...
// Store royalty information during minting
async function mint(paramAddress, paramContentTypeOrFilename, paramHexData, paramDelegateTxId = "", royaltyRecipient = "", royaltyAmount = "") {
    ...
    // Add royalty information if provided
    let royaltyData = {};
    if (royaltyRecipient && royaltyAmount) {
        let isPercentage = royaltyAmount.endsWith('%');
        let finalRoyaltyAmount = 0;

        if (isPercentage) {
            finalRoyaltyAmount = parseFloat(royaltyAmount.replace('%', ''));
        } else {
            finalRoyaltyAmount = parseFloat(royaltyAmount); // Store the flat amount
        }

        // Store the royalty data with the inscription
        royaltyData = {
            recipient: royaltyRecipient,
            amount: finalRoyaltyAmount,
            isPercentage: isPercentage
        };

        const royaltyHex = Buffer.from(JSON.stringify(royaltyData)).toString('hex');
        // Append royalty data to the inscription
        data = Buffer.concat([data, Buffer.from(royaltyHex, 'hex')]);
    }
...
// Enforce royalty during transfer
async function transferInscription(inscriptionId, newOwnerAddress, transferAmount) {
    ...
    let royaltyData = inscription.royalty;

    // Check if royalty data exists
    if (royaltyData) {
        let royaltyAmount = 0;

        if (royaltyData.isPercentage) {
            // Calculate royalty as a percentage of the transfer amount
            royaltyAmount = Math.floor(transferAmount * (royaltyData.amount / 100));
        } else {
            // Fixed amount royalty
            royaltyAmount = royaltyData.amount;
        }

        // Send the royalty amount to the royalty recipient
        await sendTransaction(royaltyData.recipient, royaltyAmount);

        // Send the remaining amount to the new owner
        let remainingAmount = transferAmount - royaltyAmount;
        await sendTransaction(newOwnerAddress, remainingAmount);
    } else {
        // If no royalty, transfer full amount
        await sendTransaction(newOwnerAddress, transferAmount);
    }
}

// Simplified transaction function
async function sendTransaction(recipient, amount) {
    let wallet = JSON.parse(fs.readFileSync(WALLET_PATH));
    let tx = new Transaction();

    // Send to recipient
    tx.to(new Address(recipient), amount);
    fund(wallet, tx);

    tx.sign(wallet.privkey);

    await broadcast(tx, true);
    console.log(`Transaction to ${recipient} for ${amount} DOGE broadcasted.`);
}

// Utility functions for formatting and sanitizing DOGE amounts
...
function sanitizeDogeInput(value = "") {
  ...
  if (newValue.length > 1 && newValue.startsWith("0") && !newValue.includes(decimalSeparator)) {
    return sanitizeDogeInput(newValue.slice(1));
  }
...
function formatSatoshisAsDoge(value, maxDecimals) {
  const newValue = sb.toBitcoin(value);
  return formatDoge(newValue, maxDecimals);
}

function formatDoge(value, maxDecimals, useGrouping) {
  const options = {};
  if (maxDecimals !== undefined) options.maximumFractionDigits = maxDecimals;
...
async function walletSync() {
  ...
  let balanceSatoshis = wallet.utxos.reduce((acc, curr) => acc + curr.satoshis, 0);
  let balanceDoge = formatSatoshisAsDoge(balanceSatoshis, 8);

  console.log('balance', balanceDoge, 'DOGE');
}
...
function walletBalance() {
  ...
  console.log(wallet.address, balanceDoge, 'DOGE');
}
